...
{ //new-orders
	...
	$.each( data, function( key, val ) {
		...
		var new_tag='';
		if (val.viewed==1){
			new_tag='<div class="new-tag">'+ getTrans('new','new') +'</div>';
		}

		htm+='<ons-col width="25%" class="center" >';
		   htm+='<div>' + new_tag;
			htm+='<ons-icon icon="'+icon_trans_type+'" class="icon"></ons-icon>';
		   htm+='</div>';
		htm+='</ons-col>';
		...
		htm+='<ons-col >';
		   htm+='<p class="margin2 concat-text bold">'+val.customer_name+'</p>';
		   htm+='<p class="margin2 ">'+ getTrans("Order No",'order_no') +'. <b>'+val.order_id+'</b></p>';
		   htm+='<p class="margin2 ">'+val.delivery_date+'</p>';
		 ...
		 htm+='<ons-col width="25%" class="text-right" >';
		   htm+='<price><b>'+val.total_w_tax_prety+'</b></price>';
		   htm+='<p class="small-font-dim orange-text">';
		   if (!empty(val.delivery_time)){
		     htm+='<ons-icon icon="fa-clock-o" class="icon"></ons-icon>';
		     htm+=' <b>'+val.delivery_time+'</b> ';
		   }
		   if (!empty(val.delivery_asap)){
		     htm+='<ons-icon icon="fa-clock" class="icon"></ons-icon>';
...
function displayNewOrders(data,div_id)
{
	...
	$.each( data, function( key, val ) {
		...
		htm+='<ons-col class="center" >';
		   htm+='<div class="delivery-icon">' + new_tag + " New Order! Click here to see. <b>"+val.total_w_tax_prety+'</b>';
...
if(time == "")
{
	...
	$.validate({
	    ...
	    borderColorOnError:"#FF0000",
	    onError : function() {
			//Sometimes the delievry time is missing, hence should not proceed further. for pickups, there is a pickup time and its not displayed automatically.
	    },
	    onSuccess : function() {
...
function playAudio(url) {
	...
	function playNewNotification(){
     ...
     my_media.play();
	 altpopup = setInterval(function(){ playNewNotification();},10000);
}
